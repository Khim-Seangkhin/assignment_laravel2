                            ...
                            <tbody>
                                ...
                                @foreach ($apply_mission_leaves as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a 
                                                href="{{ route('sale_department.approve', $item->id) }}" 
                                                class="btn btn-primary {{ $item->approver == $role ? '' : 'disabled' }}"
                                                style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;"
                                            >Approve</a>

                                            <a 
                                                href="{{ route('sale_department.reject', $item->id) }}" 
                                                class="btn btn-danger {{ $item->approver == $role ? '' : 'disabled' }}"
